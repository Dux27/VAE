...
model.load_state_dict(torch.load(os.path.join(current_path, 'weights\\vae_model_latent_200_3.pth'), weights_only=True))
model.eval()

def generate_digit(first, second, third):
    z_sample = torch.tensor([[first, second, third]], dtype=torch.float).to(device)
...
def update_digit(event=None):
    ...
    generate_digit(first, second, third)
